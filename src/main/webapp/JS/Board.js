...
// check 된 wkey 의 게시글들만 삭제하기
function chkDelete(){
	var wkeys = []; // 빈 배열 준비 
	$("#list tbody input[name=wkey]").each(function(){  // 각각에 대해 이 함수 수행하겠다.
		// $(this) //각각 name이 wkey인 checkbox 
		if($(this).is(":checked")){  // jQuery 에서 check 여부 확인방법 
			wkeys.push($(this).val()); // 배열에 wkey 값 추가 
		}
	});
	...
	if(wkeys.length == 0){
		...
		if(!confirm(wkeys.length + "개의 글을 삭제하시겠습니까?")) return false; // 아니오 선택시 false 리턴하도록.
...
// 특정 wkey 의 글 삭제하기
function deleteWkey(wkey){
	
	if(!confirm(wkey + "번의 글을 삭제 하시겠습니까?")) return false;
